...
class AudioProcessor {
  constructor(audioContext) {
    ...
    this.callbacks = {};
    this.musicMode = false; // Estado do modo música
    this.lastAnalysisTime = {}; // Timestamp da última análise por usuário
    this.analysisInterval = 100; // Intervalo mínimo entre eventos de análise (ms)
    ...
    // Conectar saída master também ao destino de gravação (para gravar áudio remoto)
  ...
  /**
   * Adiciona o stream local para gravação (sem enviar para saída de áudio para evitar eco)
  ...
  addLocalStream(stream) {
    ...
    try {
      ...
      // Conectar APENAS ao destino de gravação, não ao destination principal
      this.localSource.connect(this.recordingDestination);
      console.log('AudioProcessor: Stream local adicionado à mixagem de gravação');
    } catch (e) {
      console.error('Erro ao adicionar stream local à gravação:', e);
    }
  }

  /**
   * Inicia a gravação do áudio (Mix = Remoto + Local)
   */
  startRecording() {
    ...
    try {
      // Tentar usar codec otimizado se disponível
      ...
      console.error('MediaRecorder não suportado ou erro ao criar:', e);
      return false;
    }
    ...
    this.mediaRecorder.onstop = () => {
      ...
      const filename = `rehearsal_${timestamp}.webm`;
      
      this.triggerCallback('onRecordingFinished', { blob, url, filename });
    };

    this.mediaRecorder.start(1000); // Coletar chunks a cada 1s
    this.triggerCallback('onRecordingStarted');
    return true;
  }
  ...
  /**
   * Define o modo música (alta fidelidade)
   * @param {boolean} enabled - Se o modo música deve ser ativado
   */
  setMusicMode(enabled) {
    if (this.musicMode === enabled) return;
    
    this.musicMode = enabled;
    console.log(`AudioProcessor: Modo Música alterado para ${enabled}`);
    
    // Se ativado, forçar buffer menor para menor latência
    if (enabled) {
      // Em modo música, preferimos latência menor mesmo com risco de glitches
      // Iterar sobre todos os processadores e ajustar buffer
      this.processors.forEach((processor, userId) => {
        // Nota: ScriptProcessor não permite mudar buffer size dinamicamente sem recriar
        // Então vamos disparar uma renegociação de buffer na próxima oportunidade
        // ou forçar um ajuste imediato se a qualidade permitir
        
        // Vamos ser agressivos: tentar 256 ou 128 se possível
        this.adjustBufferSize(userId, 1.0); // Forçar reavaliação como se qualidade fosse perfeita
      });
    }
  }
  ...
  _analyzeAudioQuality(userId, buffer) {
    // Limitar frequência de eventos para evitar sobrecarga da UI
    const now = Date.now();
    if (this.lastAnalysisTime[userId] && (now - this.lastAnalysisTime[userId] < this.analysisInterval)) {
      return;
    }
    this.lastAnalysisTime[userId] = now;
  ...
  adjustBufferSize(userId, connectionQuality) {
    ...
    // Ajustar tamanho do buffer baseado na qualidade e modo música
    if (this.musicMode) {
      // Modo Música: Prioridade máxima para baixa latência
      // Tentamos manter o buffer o menor possível para garantir sincronia rítmica
      if (connectionQuality < 0.2) {
        // Conexão muito crítica, precisamos aumentar para evitar falhas graves
        newSize = 1024;
      } else if (connectionQuality < 0.5) {
        // Conexão instável
        newSize = 512;
      } else {
        // Conexão razoável a boa - latência mínima
        // 256 samples @ 48kHz ~= 5.3ms de latência de processamento
        newSize = 256;
      }
    } else {
      // Modo Voz (Padrão): Prioridade para estabilidade e clareza de fala
