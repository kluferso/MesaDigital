...
class RoomManager {
  constructor(socket) {
    ...
    this.webRTC.on('onInitialized', () => {
      if (this.webRTC.audioContext) {
        ...
        // Sincronizar modo música inicial se já estiver definido no WebRTCManager
        if (this.webRTC.musicMode) {
          this.audioProcessor.setMusicMode(true);
        }
  ...
  _setupComponentCallbacks() {
    ...
    if (this.audioProcessor) {
      ...
      this.audioProcessor.on('onRecordingStarted', () => {
        this.triggerCallback('onRecordingStarted');
      });

      this.audioProcessor.on('onRecordingFinished', (data) => {
        this.triggerCallback('onRecordingFinished', data);
      });
    }
  ...
  _setupRoomEventHandlers() {
    ...
    this.socket.on('user_joined', (data) => {
      ...
      // Se não somos nós mesmos, aguardar conexão do novo usuário (ele iniciará)
      if (user.id !== this.socket.id) {
        console.log(`Novo usuário ${user.name} entrou. Aguardando oferta de conexão...`);
        // Não iniciamos conexão aqui para evitar colisão (glare)
        // O novo usuário (joinRoom) é responsável por iniciar a conexão
      }
  ...
  async listRooms() {
    ...
    return new Promise((resolve, reject) => {
      this.socket.emit('list_rooms', (response) => {
        if (response.error) {
          reject(new Error(response.error));
          return;
        }
  ...
  /**
   * Compartilha um arquivo com a sala
   * @param {File} file - Arquivo a ser compartilhado
   */
  async shareFile(file) {
    if (!this.roomId) {
      throw new Error('Não está em nenhuma sala');
    }
    ...
    try {
      ...
      this.socket.emit('share_file', {
        ...
        type: 'pdf' // Por enquanto assumimos PDF, mas o backend valida
      });
  ...
  /**
   * Ativa/desativa o Modo Música (áudio de alta fidelidade sem processamento)
   * @param {boolean} enabled - Se o modo música deve ser ativado
   */
  setMusicMode(enabled) {
    if (!this.webRTC) {
      console.error('WebRTCManager não inicializado');
      return Promise.resolve(false);
    }

    return this.webRTC.setMusicMode(enabled);
  }
   ...
   */
  toggleVideo(enabled) {
    return this.webRTC.toggleLocalVideo(enabled);
  }
  ...
  /**
   * Inicia a gravação do áudio da sala (mixagem local + remoto)
   */
  startRecording() {
    if (!this.audioProcessor) {
      console.error('AudioProcessor não inicializado');
    ...
    return this.audioProcessor.startRecording();
  }

  /**
   * Para a gravação e retorna os dados
   */
  stopRecording() {
    if (!this.audioProcessor) {
      console.error('AudioProcessor não inicializado');
