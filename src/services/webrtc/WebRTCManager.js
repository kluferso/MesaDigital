...
class WebRTCManager {
  ...
  /**
   * Ativa ou desativa o Modo Música (áudio de alta fidelidade sem processamento)
  ...
  async setMusicMode(enabled) {
    ...
    console.log(`Alterando Modo Música para: ${enabled ? 'ATIVADO' : 'DESATIVADO'}`);
    
    // Atualizar opções de mídia
    if (enabled) {
      // Configurações para alta fidelidade musical (desliga processamentos)
      this.mediaOptions.audio = {
        echoCancellation: false,
        noiseSuppression: false,
        autoGainControl: false,
        channelCount: { ideal: 2 },
        sampleRate: { ideal: 48000 },
        latency: { ideal: 0.01 } // Tentar latência mínima
      };
    } else {
      // Configurações padrão para voz (com cancelamento de eco e ruído)
      this.mediaOptions.audio = {
        echoCancellation: { ideal: true },
        noiseSuppression: { ideal: true },
        autoGainControl: { ideal: true },
    ...
    // Se já tivermos um stream local, precisamos reinicializá-lo com as novas configurações
    if (this.localStream) {
      try {
        // Parar tracks atuais
        this.localStream.getTracks().forEach(track => track.stop());
        
        // Obter novo stream com novas configurações
        const newStream = await navigator.mediaDevices.getUserMedia(this.mediaOptions);
        console.log('Stream de áudio reinicializado com novas configurações');
        
        // Substituir tracks nas conexões existentes
        ...
        if (audioTracks.length > 0) {
          ...
          for (const [userId, connection] of this.connections) {
            ...
            if (sender) {
              console.log(`Substituindo track de áudio para conexão com ${userId}`);
              await sender.replaceTrack(audioTrack);
            }
          }
        } else {
            console.warn('Nenhum track de áudio encontrado no novo stream');
        }
        ...
        // Notificar mudança
        this.triggerCallback('onMusicModeChanged', { enabled, stream: newStream });
        
        // Atualizar processador de áudio se existir
        if (this.audioProcessor) {
          if (typeof this.audioProcessor.setMusicMode === 'function') {
            this.audioProcessor.setMusicMode(enabled);
          }
        }
      } catch (error) {
        console.error('Erro ao alternar Modo Música:', error);
        this.triggerCallback('onError', {
          type: 'music_mode_error',
          message: 'Falha ao aplicar configurações de áudio: ' + error.message,
          error
        });
  ...
  async initialize() {
    ...
    try {
      ...
      // Configurar processador de áudio se disponível
      // Nota: AudioProcessor é gerenciado externamente pelo RoomManager
  ...
  /**
   * Ativa/desativa uma faixa de vídeo local
   */
  toggleLocalVideo(enabled) {
    ...
    if (videoTracks.length === 0) {
      console.error('Nenhuma faixa de vídeo encontrada');
      return false;
    }
    
    console.log(`${enabled ? 'Ativando' : 'Desativando'} vídeo local`);
    
    videoTracks.forEach(track => {
      track.enabled = enabled;
    });
    
    this.triggerCallback('onLocalVideoToggled', { enabled });
    return true;
  }
