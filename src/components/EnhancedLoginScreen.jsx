...
export default function EnhancedLoginScreen() {
  ...
  const fetchActiveRooms = () => {
    ...
    // Fallback: listener para rooms_list caso o callback não seja suportado (compatibilidade)
    socket.once('rooms_list', (rooms) => {
      setLoadingRooms(false);
  ...
  const handleEnterRoom = async (createNewRoom = false) => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Falha de conexão';
      setError(`Erro ao entrar na sala: ${errorMessage}`);

      // Se a sala não existe, sugerir criá-la
      if (errorMessage.includes('não existe')) {
        setWarning('Esta sala ainda não foi criada. Deseja criá-la agora?');
      }
  ...
  // Função para criar a sala que não existe
  const handleCreateMissingRoom = () => {
    setWarning(null);
      ...
      >
        ...
        {warning && (
          <Alert
            ...
            action={
              warning.includes('Deseja criá-la') ? (
                <Button
                  color="inherit"
                  size="small"
                  onClick={handleCreateMissingRoom}
                >
                  Criar Sala
                </Button>
              ) : null
                  ...
                  <CardActionArea onClick={() => handleRoomSelection(activeRoom.id)}>
                    <CardContent>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Box sx={{ color: 'primary.main', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 1 }}>
                          <VisibilityIcon />
                        </Box>
                      </Box>
                    </CardContent>
